          ...
          <div class="actions">
            ...
            <table id="datatable" class="striped">
              ...
              if ($_SESSION['user_type']=='Administrator') {?>
                ...
                <tbody>
                <?php
                   $query = "SELECT * FROM ticket_t WHERE ticket_status=5";
                   $result = mysqli_query($db,$query);?>
                    <?php while($row = mysqli_fetch_assoc($result)){
                     ...
                     <tr class='clickable-row' data-href="details.php?id=<?php echo $row['ticket_id']?>">
                       <td id="type"><span class="<?php echo $class?>"> <?php echo $row['ticket_category'][0]?></span><p style="margin-top:25px;margin-bottom:-5px;font-size:8pt;">SEV<?php echo $row['severity_level']?></p> </td>
              ...
              elseif ($_SESSION['user_type']=='Network Group Manager') {?>
                  ...
                  <tbody>
                  <?php
                    $query = "SELECT * FROM ticket_t LEFT JOIN service_ticket_t USING (ticket_id) LEFT JOIN sla_t sev ON sev.id = ticket_t.severity_level LEFT JOIN ticket_status_t stat ON stat.status_id = ticket_t.ticket_status WHERE ticket_t.ticket_category='Network' AND (ticket_t.ticket_agent_id IS NULL OR stat.ticket_status=5)";
                    $result = mysqli_query($db,$query);?>
              ...
              elseif ($_SESSION['user_type'] == 'Technician') {
                $query = "SELECT * FROM ticket_t LEFT JOIN service_ticket_t USING (ticket_id) LEFT JOIN sla_t sev ON sev.id = ticket_t.severity_level LEFT JOIN ticket_status_t stat ON stat.status_id = ticket_t.ticket_status WHERE stat.ticket_status=5 AND ticket_t.ticket_agent_id = '".$_SESSION['user_id']."'";
                ?>
                <thead>
                  <tr>
                    <th></th>
